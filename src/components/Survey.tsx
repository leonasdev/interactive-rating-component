...
interface SurveyProps {
  onSubmitRating: (value: string) => void;
}
...
export default function Survey(props: SurveyProps) {
  ...
  const ratingButton = (rating: string) => {
    return (
      <label key={rating} className="relative flex justify-center items-center cursor-pointer">
        <input
          type="radio"
          name="rating"
          value={rating}
          className="appearance-none border border-gray-400 w-11 h-11 sm:w-12 sm:h-12 rounded-full hover:bg-white hover:text-black transition-colors duration-300 checked:bg-white cursor-pointer peer"
          onClick={(event) => {
      ...
      >
        <button
          ...
          onClick={(event) => {
            ...
            if (currentRating) {
              props.onSubmitRating(currentRating);
            }
          }}
        >
          Submit
